...
def test_binops():
    schema = Dict.of(
        Integer.named("main"), Ref.named("aux").to("../main"), Integer.named("other")
    )
    el = schema({"main": 5})
...
def test_writable():
    schema = Dict.of(
        Integer.named("main"), Ref.named("aux").to("../main").using(writable=True)
    )
...
def test_not_writable():
    schema = Dict.of(
        Integer.named("main"), Ref.named("aux").to("../main").using(writable=False)
    )
...
def test_dereference_twice():
    ...
    assert element.find_one(element["aux"].target_path) is element["main"]
    assert element.find_one(element["aux"].target_path) is element["main"]
