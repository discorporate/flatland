...
def test_set_default_int():
    ...
    def factory(count, **kw):
        return (
            List.named("l")
            .using(default=count, **kw)
            .of(String.named("s").using(default="val"))
        )
    ...
    schemaf = (
        List.named("l")
        .using(default_factory=calculated_default)
        .of(String.named("s").using(default="val"))
    )
...
def test_set_default_value():
    ...
    def factory(default, **kw):
        return List.using(default=default, **kw).of(String.using(default="val"))
...
def test_access():
    ...
    elements = list(Integer.named("i")(val) for val in ("10", "11", "12"))
