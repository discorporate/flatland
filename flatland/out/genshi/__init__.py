...
Input element name and value can be auto-decorated by binding the document
element to a Springy node with "bind":
...
The element will be assigned the flat-mapped name for the loginid node.
Here's another example of binding a nested list of strings to a flat form:
...
The name attribute will receive the flat name to each node using the seperator
and scoping rules in effect for the node tree, e.g.:
...
Variable expansion, <py:match> and friends happen before Springy binding is
complete.

Example:
  Imagine a <py:match> that provides <select> <option> values for each month
  of the year.  You can bind this templated element to a Springy node like so:

    <select py:match="select[@options=months]">...</select>
    <select options="months" form:bind="${form.month}"/>

  The select element will be filled out with <options> by py:match, and if one
  matches the bound element it will be selected in the generated output.

DOM ID Generation and Tabindex Management

Elements can have unique DOM IDs auto-assigned.  These are convenient for
scripting and for automatically relating <LABEL> elements to input controls.
...
DOM ID decoration defaults to on, and can be toggled via "set" or "with".  The
format for generated ID strings can also be set with "domid-format".

Tabindex Management

Elements can also have their tabindexes managed, each receiving an ascending
tabindex value.  This decoration defaults to on for bound elements.  However
any document element can be decorated with an automatic tabindex.
...
The starting tabindex can be specified via "set" or "with" using "tabindex".

Controlling and Overriding Auto-Decoration

All aspects of the form generation can be turned on and off within a template,
both on a global level and on individual fields.
...
Like the Genshi <py:with> directive, this applies the changes within a block
scope.
...
Boolean Controls:
  ...
  Turn these auto-decorations on or off.  Values can be true, false, on, off,
  yes, no, 1, or 0.
...
Element-Level Control
  ...
  Element level toggles can be controlled with a boolean value plus "maybe".
  Maybes will be applied if the document or scope-level control is true,
  otherwise not.  All bound elements have a default of "maybe" if not
  specified.

Literal Attributes

  If the source element already contains the attribute that would be
  generated, it will be left untouched by default and no generation will be
  applied.  Generation can be forced by turning on the matching element-level
  auto-* control.
